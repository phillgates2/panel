...
# Panel - Comprehensive Interactive Installer
#
...
log_info() {
    ...
    echo "[INFO] $(date): $1" >> "$LOG_FILE" 2>/dev/null || true
}

log_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1" >&2
    echo "[SUCCESS] $(date): $1" >> "$LOG_FILE" 2>/dev/null || true
}

log_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1" >&2
    echo "[WARNING] $(date): $1" >> "$LOG_FILE" 2>/dev/null || true
}

log_error() {
    echo -e "${RED}[ERROR]${NC} $1" >&2
    echo "[ERROR] $(date): $1" >> "$LOG_FILE" 2>/dev/null || true
}

log_verbose() {
    if [ "$VERBOSE" = "true" ]; then
        echo -e "${DIM}[VERBOSE]${NC} $1" >&2
        echo "[VERBOSE] $(date): $1" >> "$LOG_FILE" 2>/dev/null || true
    fi
}

log_header() {
    echo -e "\n${BOLD}${ORANGE}================================${NC}" >&2
    echo -e "${BOLD}${ORANGE} $1 ${NC}" >&2
...
check_disk_space() {
    ...
    if command -v df >/dev/null 2>&1; then
        available_space_mb=$(df -m "$INSTALL_DIR" | tail -1 | awk '{print $4}')
...
clone_repository() {
    ...
    if [ -d "$INSTALL_DIR/.git" ]; then
        ...
        if ! execute "cd '$INSTALL_DIR' && git pull origin '$BRANCH'"; then
            log_error "Failed to pull repository updates"
            exit 1
        fi
    else
        log_info "Cloning repository..."
        if ! execute "git clone -b '$BRANCH' '$REPO_URL' '$INSTALL_DIR'"; then
...
${BOLD}Next Steps:${NC}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. ${ORANGE}Start the application:${NC}
   cd $INSTALL_DIR && source venv/bin/activate && python app.py

2. ${ORANGE}Access the web interface:${NC}
   http://$DOMAIN:$APP_PORT

3. ${ORANGE}Login with admin credentials:${NC}
   Email: $ADMIN_EMAIL
   Password: $ADMIN_PASSWORD

4. ${ORANGE}Start background worker:${NC}
   python rq_worker.py
...
interactive_setup() {
    ...
    echo "This interactive installer will guide you through the installation process."
    echo

    # Installation mode
    local modes=("Development (SQLite, debug mode)" "Production (PostgreSQL, optimized)" "Custom (choose components)")
    show_menu "Select Installation Mode:" "${modes[@]}"
    local mode_choice=$?
    ...
    if [ "$DB_TYPE" = "postgresql" ]; then
        ...
        DB_PASS=$(get_input "Database password" "" "true")
        if [ -z "$DB_PASS" ]; then
    ...
    ADMIN_PASSWORD=$(get_input "Admin password" "" "true")
    ...
    DOMAIN=$(get_input "Domain name" "localhost")
    APP_PORT=$(get_input "Application port" "8080")

    # SSL configuration
    if [ "$ENABLE_SSL" != "false" ]; then
        if confirm "Enable SSL/TLS?" "y"; then
            ENABLE_SSL=true
            if confirm "Use Let's Encrypt for automatic SSL certificates?" "y"; then
                ENABLE_LETSENCRYPT=true
            fi
        else
            ENABLE_SSL=false
    ...
    if [ "$SETUP_SYSTEMD" != "false" ]; then
        if confirm "Setup systemd services for auto-start?" "y"; then
            SETUP_SYSTEMD=true
        else
            SETUP_SYSTEMD=false
        fi
    ...
    if [ "$ENABLE_MONITORING" != "false" ]; then
        if confirm "Enable monitoring (Prometheus/Grafana)?" "y"; then
            ENABLE_MONITORING=true
        else
            ENABLE_MONITORING=false
        fi
    ...
    # Backup configuration
    if [ "$ENABLE_BACKUPS" != "false" ]; then
        if confirm "Enable automated backups?" "y"; then
            ENABLE_BACKUPS=true
            local schedules=("Daily" "Weekly" "Hourly")
            show_menu "Select backup schedule:" "${schedules[@]}"
            local schedule_choice=$?
            case $schedule_choice in
                0) BACKUP_SCHEDULE="daily" ;;
                1) BACKUP_SCHEDULE="weekly" ;;
...
configure_custom_mode() {
    ...
    # Database type
    local db_types=("PostgreSQL (recommended for production)" "SQLite (simple, no setup required)")
    show_menu "Select database type:" "${db_types[@]}"
    local db_choice=$?
    case $db_choice in
        0) DB_TYPE="postgresql" ;;
        1) DB_TYPE="sqlite" ;;
    esac
    
    # Components
    if confirm "Install system dependencies?" "y"; then
        SKIP_DEPS=false
    else
        SKIP_DEPS=true
    fi
    
    if confirm "Setup PostgreSQL database?" "y"; then
        SKIP_POSTGRESQL=false
    else
        SKIP_POSTGRESQL=true
    fi
    
    if confirm "Setup Redis cache?" "y"; then
        SKIP_REDIS=false
    else
        SKIP_REDIS=true
    fi
    ...
    if confirm "Setup systemd services?" "n"; then
        SETUP_SYSTEMD=true
    fi
    ...
    if confirm "Enable automated backups?" "n"; then
        ENABLE_BACKUPS=true
    fi
}
...
main() {
    ...
    # Check if dry run
    if [ "$DRY_RUN" = "true" ]; then
        log_warning "DRY RUN MODE: No actual changes will be made. This is a simulation."
    fi

    # Initialize log file
    if [ "$DRY_RUN" != "true" ]; then
        mkdir -p "$(dirname "$LOG_FILE")" 2>/dev/null || true
        echo "Installation started at $(date)" > "$LOG_FILE" 2>/dev/null || log_warning "Cannot write to log file: $LOG_FILE"
    fi
...
show_help() {
    ...
    echo ""
    echo "Install or update the panel application."
    echo ""
    echo "Options:"
    ...
    echo "  --save-secrets         Save secrets to file"
    echo "  --redact-secrets       Redact secrets in output"
    echo "  --config-only          Only generate configuration"
    echo "  --no-pip-install       Skip pip package installation"
    echo "  --force                Force installation (overwrite existing)"
    echo "  --verbose              Enable verbose output"
    echo "  --dry-run              Show what would be done without making changes"
    echo ""
    echo "Environment variables:"
    echo "  PANEL_INSTALL_DIR      Installation directory (default: ~/panel)"
    echo "  PANEL_BRANCH           Git branch to use (default: main)"
    echo "  PANEL_*                Various flags (see above)"
}
