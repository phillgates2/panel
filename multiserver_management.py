...
class LoadBalancer(db.Model):
    ...
    )  # round_robin, least_players, weighted
...
class MultiServerManager:
    ...
    def sync_configuration(
        self, source_server_id, target_server_ids, sync_type="config", user_id=None
    ):
        ...
        try:
            ...
            sync_thread = threading.Thread(
                target=self._execute_sync, args=(sync_record.id,), daemon=True
            )
            sync_thread.start()
    ...
    def update_player_session(self, server_id, player_name, player_guid, session_data):
