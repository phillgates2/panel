...
class PlayerBan(db.Model):
    ...
    server_id = db.Column(db.Integer, db.ForeignKey('server.id'), nullable=True)  # NULL for global ban
...
class PlayerReport(db.Model):
    ...
    reported_player = db.relationship('Player', backref=db.backref('reports_against', lazy='dynamic'))
    server = db.relationship('Server')
...
class PlayerManager:
    ...
    def ban_player(self, player_id, server_id, ban_type, reason, duration_hours=None, 
                   banned_by_user_id=None, evidence=None):
        """Ban a player."""
        try:
            expires_at = None
            if duration_hours:
    ...
    def create_report(self, reported_player_id, server_id, report_type, description,
                     reporter_name=None, reporter_guid=None, reporter_ip=None, evidence=None):
    ...
    def get_player_statistics(self, player_id):
        ...
        # Recent sessions
        recent_sessions = PlayerSession.query.filter_by(
            player_id=player_id
        ).order_by(desc(PlayerSession.started_at)).limit(10).all()
        ...
        ).filter(PlayerSession.player_id == player_id).group_by(PlayerSession.server_id).all()
...
def players_dashboard():
    ...
    return render_template('admin_players_dashboard.html',
...
def reports_dashboard():
    ...
    reports = PlayerReport.query.filter_by(status=status_filter).order_by(
        desc(PlayerReport.reported_at)
    ).limit(100).all()
    
    return render_template('admin_player_reports.html', 
                         reports=reports, 
...
def search_players():
    ...
    if query_text:
        query = query.filter(
            or_(
                Player.name.contains(query_text),
                Player.guid.contains(query_text)
            )
        )
