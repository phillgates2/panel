...
class ConfigDeployment(db.Model):
    ...
    deployment_status = db.Column(db.String(32), nullable=False)  # pending, success, failed, rollback
    ...
    config_version = db.relationship('ConfigVersion', backref=db.backref('deployments', lazy='dynamic'))
...
class ConfigManager:
    ...
    def create_version(self, config_data, user_id, change_summary=None):
        ...
        last_version = ConfigVersion.query.filter_by(
            server_id=self.server_id
    ...
    def get_version_history(self, limit=50):
        ...
        return ConfigVersion.query.filter_by(
            server_id=self.server_id
        ).order_by(ConfigVersion.created_at.desc()).limit(limit).all()
