...
interactive_config() {
    ...
    if [[ "$DB_TYPE" == "mysql" ]]; then
        if [[ "$INSTALL_MODE" == "production" ]] || [[ "$INSTALL_MODE" == "custom" ]]; then
            if prompt_confirm "Install phpMyAdmin for database management? (recommended for production)" "y"; then
                SETUP_PHPMYADMIN="true"
            else
                SETUP_PHPMYADMIN="false"
            fi
        else
            if prompt_confirm "Install phpMyAdmin for database management?" "n"; then
                SETUP_PHPMYADMIN="true"
            else
                SETUP_PHPMYADMIN="false"
            fi
    ...
    echo -e "    phpMyAdmin: ${SETUP_PHPMYADMIN:-false}"
...
install_system_deps() {
    ...
    if [[ "${SETUP_PHPMYADMIN:-false}" == "true" ]]; then
        case "$PKG_MANAGER" in
            apt-get) packages="$packages phpmyadmin php-mbstring php-zip php-gd php-json php-curl";;
            yum) packages="$packages phpmyadmin php-mbstring php-zip php-gd php-json php-curl";;
            apk) packages="$packages phpmyadmin php-mbstring php-zip php-gd php-json php-curl";;
        esac
    fi
...
setup_phpmyadmin() {
    ...
    # Detect package manager and configure phpMyAdmin
    if command -v apt-get &> /dev/null; then
        # Debian/Ubuntu specific configuration
        log "Configuring phpMyAdmin for Debian/Ubuntu..."
        
        # Create phpMyAdmin configuration directory if it doesn't exist
        local pma_config_dir="/etc/phpmyadmin"
        if [[ $EUID -eq 0 ]]; then
            mkdir -p "$pma_config_dir"
        else
            sudo mkdir -p "$pma_config_dir"
        fi
        
        # Generate blowfish secret for phpMyAdmin
        local blowfish_secret=$(openssl rand -hex 32 2>/dev/null || head -c 32 /dev/urandom | xxd -p)
        
        # Create phpMyAdmin configuration
        cat > /tmp/config.inc.php << EOF
<?php
/**
 * phpMyAdmin configuration for Panel
 * Auto-generated by Panel installer
 */
...
EOF
        ...
        # Copy configuration to phpMyAdmin config directory
        if [[ $EUID -eq 0 ]]; then
            cp /tmp/config.inc.php "$pma_config_dir/config.inc.php"
            chmod 640 "$pma_config_dir/config.inc.php"
            chown root:www-data "$pma_config_dir/config.inc.php"
        else
            sudo cp /tmp/config.inc.php "$pma_config_dir/config.inc.php"
            sudo chmod 640 "$pma_config_dir/config.inc.php"
            sudo chown root:www-data "$pma_config_dir/config.inc.php"
        fi
        
        rm -f /tmp/config.inc.php
        
        # Create nginx configuration for phpMyAdmin if nginx is being set up
        if [[ "$SETUP_NGINX" == "true" ]]; then
            log "Configuring Nginx for phpMyAdmin access..."
...
EOF
            ...
            # Create a separate phpMyAdmin nginx config file
            if [[ $EUID -eq 0 ]]; then
                cp /tmp/phpmyadmin.conf /etc/nginx/conf.d/phpmyadmin.conf
            else
                sudo cp /tmp/phpmyadmin.conf /etc/nginx/conf.d/phpmyadmin.conf
            fi
            
            rm -f /tmp/phpmyadmin.conf
            
            # Enable and restart PHP-FPM
            if systemctl list-unit-files | grep -q php.*fpm; then
                local php_fpm_service=$(systemctl list-unit-files | grep php.*fpm | awk '{print $1}' | head -n1)
                log "Enabling PHP-FPM service: $php_fpm_service"
                if [[ $EUID -eq 0 ]]; then
            ...
            log "✓ phpMyAdmin configured with Nginx at /phpmyadmin"
        else
            log "✓ phpMyAdmin installed (manual web server configuration required)"
        fi
        
    elif command -v yum &> /dev/null; then
        # RedHat/CentOS configuration
        log "Configuring phpMyAdmin for RedHat/CentOS..."
        warn "phpMyAdmin requires manual configuration on RHEL/CentOS systems"
        warn "Please configure /etc/phpMyAdmin/config.inc.php and web server manually"
        
    elif command -v apk &> /dev/null; then
        # Alpine configuration
        log "Configuring phpMyAdmin for Alpine..."
        warn "phpMyAdmin requires manual configuration on Alpine systems"
        warn "Please configure /etc/phpmyadmin/config.inc.php and web server manually"
    fi
    
    log "✓ phpMyAdmin setup complete"
}
...
show_next_steps() {
    ...
    # Show phpMyAdmin access if installed
    if [[ "${SETUP_PHPMYADMIN:-false}" == "true" ]]; then
        echo -e "${BLUE}Database Management (phpMyAdmin):${NC}"
        if [[ "$SETUP_NGINX" == "true" ]]; then
            if [[ "$SETUP_SSL" == "true" ]]; then
                echo "  URL: https://$DOMAIN_NAME/phpmyadmin"
            else
                echo "  URL: http://$DOMAIN_NAME/phpmyadmin"
            fi
        else
...
show_help() {
    ...
    echo "  • Optional Features: Redis, phpMyAdmin, Discord webhooks, ML dependencies"
