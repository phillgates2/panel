"""
Tests for Database Admin Integration
"""
...
class TestDatabaseSecurity:
    ...
    def test_read_only_query_detection(self):
        ...
        for query in read_only:
            assert DatabaseSecurity.is_read_only_query(
                query
            ), f"Failed to identify as read-only: {query}"

        for query in read_write:
            assert not DatabaseSecurity.is_read_only_query(
                query
            ), f"Incorrectly identified as read-only: {query}"
...
class TestDatabaseAdmin:
    """Test Database Admin integration (requires test database)"""
