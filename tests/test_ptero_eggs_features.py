...
from ptero_eggs_updater import (PteroEggsTemplateVersion, PteroEggsUpdateMetadata,
                                PteroEggsUpdater)
...
def test_ptero_eggs_browser_route_exists():
    ...
    assert 'config.ptero_eggs_browser' in route_names or any('browser' in name for name in route_names)
...
def test_ptero_eggs_routes_registered():
    ...
    for func_name in expected_functions:
        ...
        found = (
            hasattr(config_bp, func_name) or
            func_name in dir(config_bp) or
            any(func_name in str(rule) for rule in config_bp.deferred_functions)
        )
        # At minimum, the module should have these functions defined
        import routes_config
        assert hasattr(routes_config, func_name) or func_name in routes_config.config_bp.view_functions.get(f'config.{func_name}', {})
