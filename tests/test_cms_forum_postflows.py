...
def test_admin_login_and_cms_create(client, app):
    ...
    rv = client.post(
        "/cms/admin/login",
        data={"username": user_email, "password": "secret"},
    ...
    rv2 = client.post(
        "/cms/create",
        data={"title": "Test Page", "slug": "test-page", "content": "Hello world"},
        follow_redirects=True,
    )
...
def test_forum_thread_reply_edit_delete(client, app):
    ...
    rv2 = client.post(
        "/forum/thread/create",
        data={"title": "Thread One", "author": "Admin", "content": "First post"},
        follow_redirects=True,
    )
    assert rv2.status_code == 200
    ...
    # reply to the thread
    rv3 = client.post(
        f"/forum/thread/{tid}/reply",
        data={"author": "Replier", "content": "Reply content"},
        follow_redirects=True,
    )
    assert rv3.status_code == 200
