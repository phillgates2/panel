...
class LogProcessor:
    ...
    def _load_patterns(self):
        ...
        try:
            patterns = (
                LogPattern.query.filter_by(is_active=True)
                .order_by(LogPattern.priority.desc())
                .all()
            )
    ...
    def _process_log_line(self, log_data):
        ...
        # Store in database
        try:
            log_entry = LogEntry(server_id=server_id, **parsed_entry)
    ...
    def _send_discord_alert(self, alert, server, message):
        ...
        try:
            ...
            embed = {
                ...
                "fields": [
                    {"name": "Alert", "value": alert.name, "inline": True},
    ...
    def get_log_statistics(self, server_id=None, hours=24):
        ...
        event_stats = db.session.query(
            LogEntry.event_type, func.count(LogEntry.id).label("count")
        ...
        level_stats = db.session.query(
            LogEntry.log_level, func.count(LogEntry.id).label("count")
        ).filter(LogEntry.timestamp >= since)
...
def logs_dashboard():
    ...
    return render_template("admin_logs_dashboard.html", servers=servers, stats=stats)
...
def server_logs(server_id):
    ...
    # Get recent logs
    logs = (
        LogEntry.query.filter_by(server_id=server_id)
...
def search_logs():
    ...
    return jsonify({"success": True, "logs": results, "total": len(results)})
