...
class BackupManager:
    ...
    def create_server_backup(self, server_id, server_data, name=None):
        ...
        try:
            ...
            backup_data = {
                ...
                "metadata": {"version": "1.0", "type": "server_config"},
            }
    ...
    def restore_config(self, backup_file):
        ...
        try:
            ...
            with tarfile.open(backup_path, "r:gz") as tar:
                # Validate tarfile members for security
                for member in tar.getmembers():
                    # Prevent path traversal attacks
                    if os.path.isabs(member.name) or ".." in member.name:
                        raise ValueError(f"Unsafe path in tarfile: {member.name}")
                    # Prevent extremely long paths
                    if len(member.name) > 255:
                        raise ValueError(f"Path too long in tarfile: {member.name}")
    ...
    def get_backup_stats(self):
        ...
        for backup_type, backup_dir in [
            ...
            type_stats = {"count": 0, "size": 0, "oldest": None, "newest": None}
