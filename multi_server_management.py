...
class ServerManager:
    ...
    def start_server(self, node: ServerNode) -> bool:
        ...
        success, stdout, stderr = self.execute_command(
            node, 
            f"systemctl start etlegacy-{node.server.name}"
        )
    ...
    def stop_server(self, node: ServerNode) -> bool:
        ...
        success, stdout, stderr = self.execute_command(
            node, 
            f"systemctl stop etlegacy-{node.server.name}"
        )
...
class ClusterManager:
    ...
    def auto_scale_cluster(self, cluster_id: int):
        ...
        if stats.avg_cpu > cluster.target_cpu_utilization and stats.online_servers < cluster.max_servers:
            self._scale_up_cluster(cluster)
        
        # Scale down if CPU usage is low and we have more than minimum servers
        elif stats.avg_cpu < (cluster.target_cpu_utilization * 0.5) and stats.online_servers > cluster.min_servers:
    ...
    def _scale_up_cluster(self, cluster: ServerCluster):
        ...
        offline_node = db.session.query(ServerNode).filter_by(
    ...
    def _scale_down_cluster(self, cluster: ServerCluster):
        ...
        online_nodes = db.session.query(ServerNode).filter_by(
            cluster_id=cluster.id,
            status=ServerStatus.ONLINE
        ).all()
...
def cluster_dashboard():
    ...
    recent_deployments = db.session.query(ServerDeployment).order_by(
        desc(ServerDeployment.created_at)
    ).limit(10).all()
    
    return render_template('admin_multi_server.html', 
...
def cluster_detail(cluster_id):
    ...
    deployments = db.session.query(ServerDeployment).filter_by(
        cluster_id=cluster_id
    ).order_by(desc(ServerDeployment.created_at)).limit(20).all()
