...
def send_bulk_notifications(user_ids: list, subject: str, message: str, notification_type: str = 'email') -> Dict[str, Any]:
    ...
    try:
        ...
        with app.app_context():
            ...
            job.meta['progress'] = 100
            job.meta['message'] = f'Bulk notification completed: {successful} sent, {failed} failed'
            job.save_meta()
    ...
    try:
        ...
        with app.app_context():
            for i, server_id in enumerate(server_ids):
                ...
                job.meta['progress'] = progress
                job.meta['message'] = f'Updated metrics for {i + 1}/{total_servers} servers'
                job.save_meta()

            job.meta['progress'] = 100
            job.meta['message'] = f'Metrics update completed: {successful} successful, {failed} failed'
            job.save_meta()
...
class JobManager:
    ...
    def enqueue_bulk_notification(self, user_ids: list, subject: str, message: str) -> str:
    ...
    def get_job_status(self, job_id: str, queue_name: str = 'default') -> Optional[Dict[str, Any]]:
        """Get status of a job."""
        try:
