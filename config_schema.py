...
class DatabaseConfig(BaseModel):
    ...
    sqlite_uri: str = Field(default="sqlite:///panel_dev.db", description="SQLite database URI")
    postgres_user: str = Field(default="paneluser", description="PostgreSQL username")
    postgres_password: str = Field(default="panelpass", description="PostgreSQL password")
    ...
    postgres_database: str = Field(default="paneldb", description="PostgreSQL database name")

    # Connection pooling settings
    pool_pre_ping: bool = Field(default=True, description="Verify connections before use")
    pool_recycle: int = Field(default=300, ge=0, description="Recycle connections after N seconds")
...
class LoggingConfig(BaseModel):
    ...
    directory: Path = Field(default_factory=lambda: Path("instance/logs"), description="Log directory")
    ...
    performance_threshold: float = Field(default=500.0, ge=0, description="Slow request threshold (ms)")
...
class ETLegacyConfig(BaseModel):
    ...
    server_port: int = Field(default=27960, ge=1, le=65535, description="ET:Legacy server port")
    rcon_password: str = Field(default="changeme", min_length=1, description="RCON password")
    pid_file: Path = Field(
...
class SecurityConfig(BaseModel):
    ...
    admin_emails: List[str] = Field(default_factory=list, description="Admin email addresses")
...
class SystemConfig(BaseModel):
    ...
    os_system: str = Field(default_factory=lambda: os_paths.system, description="Operating system")
    os_distro: str = Field(default_factory=lambda: os_paths.distro, description="OS distribution")
...
class PanelConfig(BaseSettings):
    ...
    def validate_database_config(self) -> 'PanelConfig':
        ...
        if not self.database.use_sqlite:
            ...
            required_fields = ['postgres_user', 'postgres_password', 'postgres_host', 'postgres_database']
            for field in required_fields:
                value = getattr(self.database, field)
                if not value or str(value).strip() == "":
                    raise ValueError(f"PostgreSQL {field} is required when not using SQLite")
        return self
...
def generate_config_template(output_file: Union[str, Path]) -> None:
    ...
    default_config = PanelConfig(security=SecurityConfig(secret_key="your-secret-key-here"))
