...
class ServerAlert(db.Model):
    ...
    )  # NULL for global alerts
    alert_type = db.Column(
        db.String(64), nullable=False
    )  # cpu_high, memory_high, server_offline, etc.
    ...
    def _ping_server(self, server):
        ...
        try:
            ...
            result = subprocess.run(
                ["nc", "-z", "-w", "3", host, str(port)], capture_output=True, timeout=5
            )
        ...
        try:
            ...
            for alert in alerts:
                ...
                if alert.alert_type == "cpu_high":
                    ...
                    if metric_value and self._compare_values(
                        metric_value, alert.comparison_operator, alert.threshold_value
                    ):
                        should_trigger = True
    ...
    def _trigger_alert(self, alert, server, metric_value, metrics):
        ...
        try:
            ...
            severity = self._determine_severity(
                alert.alert_type, metric_value, alert.threshold_value
            )
...
def monitoring_dashboard():
    ...
    return render_template("admin_monitoring_dashboard.html", server_stats=server_stats)
...
def server_monitoring_detail(server_id):
    ...
    # Get recent alerts
    alerts = (
        AlertHistory.query.filter(AlertHistory.server_id == server_id)
    ...
    player_sessions = (
        PlayerSession.query.filter(PlayerSession.server_id == server_id)
        .order_by(desc(PlayerSession.session_start))
        .limit(50)
        .all()
    )

    return render_template(
        "admin_monitoring_server_detail.html",
        server=server,
        metrics=metrics,
        alerts=alerts,
...
def live_monitoring_data(server_id):
    ...
    return jsonify({"success": True, "data": live_data, "timestamp": time.time()})
