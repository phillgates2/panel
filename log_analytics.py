...
class AnomalyDetector:
    ...
    def update_baseline(self, server_id: int, log_entries: List[LogEntry]):
        ...
        if hourly_counts:
            self.baseline_stats[server_id]["avg_messages_per_hour"] = statistics.mean(
    ...
    def detect_anomalies(
        self, server_id: int, recent_entries: List[LogEntry]
    ) -> List[Tuple[LogEntry, float, str]]:
...
def log_analytics_dashboard():
    ...
    for server in servers:
        ...
        # Alert count
        alert_count = (
            db.session.query(func.count(LogAlert.id))
            .filter(LogAlert.server_id == server.id, LogAlert.resolved_at.is_(None))
            .scalar()
        )
...
def server_log_analysis(server_id):
    ...
    recent_logs = (
        db.session.query(LogEntry)
        .filter(LogEntry.server_id == server_id, LogEntry.timestamp > cutoff_time)
    ...
    # Anomalies
    anomalies = (
        db.session.query(LogAnomaly)
        .filter(LogAnomaly.server_id == server_id, LogAnomaly.created_at > cutoff_time)
    ...
    # Alerts
    alerts = (
        db.session.query(LogAlert)
        .filter(LogAlert.server_id == server_id, LogAlert.created_at > cutoff_time)
        .order_by(desc(LogAlert.created_at))
        .all()
    )

    return render_template(
        "server_log_analysis.html",
        server=server,
