...
from app import db
from monitoring_system import ServerAlert, ServerMetrics
...
def get_dashboard_metrics():
    ...
    try:
        ...
        cpu_timeline = (
            ...
            .group_by(func.strftime("%Y-%m-%d %H:%M", ServerMetrics.timestamp))
        ...
        online_count = (
            ...
            .filter(ServerMetrics.is_online)
            .count()
        )
...
def get_recent_alerts():
    ...
    try:
        ...
        alerts = (
            db.session.query(
                ServerAlert,
                db.session.query(db.text("servers.name"))
                .filter(db.text("servers.id = server_alerts.server_id"))
                .scalar_subquery()
                .label("server_name"),
            )
            .filter(
...
def resolve_alert(alert_id):
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"success": False, "message": f"Error resolving alert: {str(e)}"}),
            500,
        )
...
def get_system_status():
    ...
    try:
        ...
        total_players = (
            db.session.query(func.sum(ServerMetrics.player_count))
            .join(
                latest_metrics_subq,
                (ServerMetrics.server_id == latest_metrics_subq.c.server_id)
                & (ServerMetrics.timestamp == latest_metrics_subq.c.latest_timestamp),
