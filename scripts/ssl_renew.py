    ...
    def check_certificate_expiry(self, domain):
        ...
        try:
            cmd = [
                ...
                '-in', f'/etc/letsencrypt/live/{domain}/cert.pem'
            ]
...
"""
        ...
        logger.info(f"SSL renewal check completed. Renewed: {len(renewed_domains)}, Failed: {len(failed_domains)}")
        ...
        if args.dry_run:
            ...
            for domain in manager.config['domains']:
                ...
                if expiry:
                    ...
                    status = "NEEDS RENEWAL" if days_left <= manager.config['days_before_expiry'] else "OK"
