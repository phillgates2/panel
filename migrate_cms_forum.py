    ...
    with app.app_context():
        ...
        if 'author_id' not in thread_columns:
            ...
            with db.engine.connect() as conn:
                ...
                conn.execute(text('ALTER TABLE forum_thread ADD FOREIGN KEY (author_id) REFERENCES user(id)'))
                conn.commit()
        ...
        if 'is_pinned' not in thread_columns:
            ...
            with db.engine.connect() as conn:
                conn.execute(text('ALTER TABLE forum_thread ADD COLUMN is_pinned BOOLEAN DEFAULT 0'))
                conn.commit()
        ...
        if 'is_locked' not in thread_columns:
            ...
            with db.engine.connect() as conn:
                conn.execute(text('ALTER TABLE forum_thread ADD COLUMN is_locked BOOLEAN DEFAULT 0'))
                conn.commit()
        ...
        if 'author_id' not in post_columns and 'author' in post_columns:
            ...
            with db.engine.connect() as conn:
                ...
                print("⚠ Warning: Existing posts with string authors will have author_id set to NULL")
                print("  You may want to manually update these or delete old posts")
                
                # Add foreign key constraint
                conn.execute(text('ALTER TABLE forum_post ADD FOREIGN KEY (author_id) REFERENCES user(id)'))
