...
def admin_system_dashboard():
    ...
    # Recent activity
    recent_logins = (
        UserActivity.query.filter_by(activity_type="login")
    ...
    return jsonify(
        {
            ...
            "services": {"gunicorn": gunicorn_status, "worker": worker_status},
            "database": {
...
def enable_2fa():
    ...
    provisioning_uri = totp.provisioning_uri(name=user.email, issuer_name="Panel")
    ...
    return render_template("account_2fa_setup.html", secret=secret, qr_code=qr_data)
...
def server_templates():
    ...
    templates = (
        ServerTemplate.query.filter_by(is_public=True)
        .order_by(ServerTemplate.use_count.desc())
        .all()
    )
...
@app.route("/api/admin/security-report")
@require_system_admin
...
def security_settings():
    """Security settings management for administrators."""
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            app.logger.error(f"Error updating security settings: {e}")
