...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('PANEL_SQLITE_URI', 'sqlite:///panel_dev.db')
app.config['SQLALCHEMY_POOL_SIZE'] = int(os.environ.get('PANEL_DB_POOL_SIZE', '10'))
app.config['SQLALCHEMY_MAX_OVERFLOW'] = int(os.environ.get('PANEL_DB_MAX_OVERFLOW', '20'))
...
# Configure logging
from src.panel.structured_logging import log_security_event, setup_structured_logging

logger = setup_structured_logging(app)

# Configure enhanced security hardening
from src.panel.security_hardening import init_security_hardening  # noqa: E402

security_hardening = init_security_hardening(app)
try:
    from src.panel.tools.mail import mail  # noqa: E402
...
from src.panel.rate_limiting import setup_rate_limiting, init_rate_limiting_admin
...
from src.panel.ai_integration import init_ai_features, get_ai_client, get_content_moderator, get_ai_assistant
init_ai_features()
...
def create_app(config_obj=None):
    ...
    from src.panel.security_headers import configure_security_headers  # noqa: E402
...
try:
    from src.panel.cms import cms_bp  # type: ignore
except Exception:
    cms_bp = None
try:
    from src.panel.forum import forum_bp  # type: ignore
except Exception:
    forum_bp = None

try:
    from src.panel.admin import admin_bp  # type: ignore
except Exception:
    admin_bp = None
