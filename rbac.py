...
class UserRole(db.Model):
    ...
    user = db.relationship(
        "User", foreign_keys=[user_id], backref=db.backref("user_roles", lazy="dynamic")
    )
...
def has_permission(user, permission_name):
    ...
    override = (
        UserPermissionOverride.query.filter_by(user_id=user.id)
        .join(Permission)
...
def create_default_roles():
    ...
    default_roles = [
        ("Super Administrator", "Full system access", True, ["admin.full_access"]),
        (
            "Administrator",
            "System administration with limited access",
            True,
            [
        ...
        ("User", "Basic user access", True, ["user.view_own", "server.view_assigned"]),
    ]
