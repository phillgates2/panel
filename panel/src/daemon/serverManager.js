...
async function installServer(serverId) {
  try {
    const serverRes = await query('SELECT s.*, e.install_script, e.config_files FROM servers s JOIN eggs e ON s.egg_id = e.id WHERE s.id = $1', [serverId]);
    ...
    const configFiles = typeof server.config_files === 'string' ? JSON.parse(server.config_files) : server.config_files;
    ...
    addConsoleLog(server.uuid, `[Installation] Verification script active...`);
...
async function startServer(serverId) {
  const serverRes = await query('SELECT s.*, e.startup_command FROM servers s JOIN eggs e ON s.egg_id = e.id WHERE s.id = $1', [serverId]);
...
async function sendCommand(serverId, command) {
  const serverRes = await query('SELECT s.*, e.name AS egg_name FROM servers s JOIN eggs e ON s.egg_id = e.id WHERE s.id = $1', [serverId]);
  if (serverRes.rows && serverRes.rows.length === 0) throw new Error('Server not found');
  const server = serverRes.rows[0];
